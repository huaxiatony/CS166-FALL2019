...
public class EmbeddedSQL {
   ...
   public static void Query1(EmbeddedSQL esql){
       try{
	   String query = "SELECT COUNT(C.pid), C.sid FROM catalog C GROUP BY C.sid;";
	   int rowCount = esql.executeQuery(query);
	   System.out.println("total row(s): " + rowCount);
       }
       catch(Exception e){
   ...
   public static void Query2(EmbeddedSQL esql){
       try{
	   String query = "SELECT COUNT(C.pid), S.sid, S.sname FROM Suppliers S, Catalog C WHERE S.sid = C.sid GROUP BY S.sid HAVING COUNT(C.pid) >=3;";
   ...
   public static void Query3(EmbeddedSQL esql){
       try{
	   String query = "SELECT S.sid, S.sname, COUNT(*) FROM suppliers S, parts P, catalog C WHERE S.sid IN (SELECT S.sid FROM suppliers S WHERE S.sid NOT IN (SELECT C.sid FROM catalog C, parts P WHERE C.pid = P.pid AND p.color != 'Green')) AND S.sid = C.sid AND P.pid =C.pid GROUP BY S.sid;";
	   int output = esql.executeQuery(query);
	   System.out.println ("total row(s): " + output);
       }
       catch(Exception e){
